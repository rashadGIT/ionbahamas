...
import { getCause } from '../module/walk-a-thon'
import { checkBox, checkBoxLabel, title,  } from '../css/input.css'
import Payments from '../components/Payment/Payments';
import { moneyFormat } from '../module/util'
import { donationCardNonceResponseReceived, eventCardNonceResponseReceived } from '../module/square';
import { isValidDonation } from '../module/validator'
import { error } from '../css/style.css.js'
import CheckAmountBox from '../components/CheckBox/CheckAmountBox'
...
export default function EventForm (props) {
  ...
  const addGuestPrice = (price) => {
    var keys = Object.keys(price);
    var value = price[keys[0]];
    var temp = guest;
    temp[keys[0]] = value;
    setGuest(temp)
    var values = Object.keys(guest).map(function(v) { return guest[v]; });
    setAmount(values.reduce(reducer))
  }
        ...
        <Container fluid={true} style={{paddingTop : '30px'}}>
              ...
              <Row>
                <Col style={{paddingTop : '0px'}}>
                  <center>
                    ...
                    /> */}
                    <Row>
                        {cause.suggestedDonations.slice(0,3).map((x) => CheckAmountBox(x,amount,addGuestPrice))}
                    </Row>
                    {/* </FormGroup> */}
                  </center>
                </Col>
          ...
          <Row>
            <Col  xs={12} md={12} lg={12}>
              <Payments
                id={10}
                btnText={`Pay ${moneyFormatStr} with card`}
                validator={isValidDonation}
                setInputError={setInputError}
                cardNonceResponseReceived={eventCardNonceResponseReceived}
                formData={{
                    amount : amount,
